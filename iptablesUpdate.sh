...
if [ $ruleUpdateFlag -eq '1' ]; then
	...
	while IFS= read -r line; do
		#remove newline chars from line
		line=$(echo $line | tr -d '\r\n')
	...
	echo 0 | ssh testuser01@ec2-3-86-210-241.compute-1.amazonaws.com 'cat >| RULES/ruleCheck.txt'
       #get date and format, then write to log file that rules were applied
	timeStamp=$(TZ='America/New_York' date +"%b %d %H:%M")
 	echo "$timeStamp Filtering Change Applied" | ssh testuser01@ec2-3-86-210-241.compute-1.amazonaws.com 'cat >> /home/testuser01/LOG/log.txt'	
fi
